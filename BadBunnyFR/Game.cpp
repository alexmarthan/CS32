...
///AUXILIARY FUNCTIONS ONLY USED IN GAME
...
string Game::takePlayerTurn()
{
    for (;;)
    {
        ...
        string playerMove; // replace with equal to h for debugging purposes
        ...
        else if (playerMove.size() == 1)
        {
            if (tolower(playerMove[0]) == 'c')
                return player->dropPoisonedCarrot();
            else if (tolower(playerMove[0]) == 'h'){ // my part
                m_arena->history().display(); // display the grid
...
void Game::play()
{
    ...
    while ( ! player->isDead()  &&  m_arena->rabbitCount() > 0)
    {
        ...
        if (msg == "Press enter to continue."){ // my added part
            cin.ignore(1000,'\n');
            m_arena->display(" "); // originally printed message
        }
        else{ // i put everything beneath in an else statement
            m_arena->display(msg);
            if (player->isDead())
                break;
            m_arena->moveRabbits();
